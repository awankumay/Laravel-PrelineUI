...
<body class="bg-gray-50 transition-all duration-300 lg:hs-overlay-layout-open:ps-65 dark:bg-neutral-900">
    <!-- Breadcrumb -->
    <div
        class="sticky top-0 inset-x-0 z-20 bg-white border-y border-gray-200 px-4 sm:px-6 lg:px-8 dark:bg-neutral-800 dark:border-neutral-700">
            ...
            <nav
                ...
                <div class="hs-accordion-group pb-0 px-2  w-full flex flex-col flex-wrap" data-hs-accordion-always-open>
                    <ul class="space-y-1">
                        ...
                        {{-- Accordion Menu --}}
                        <li class="hs-accordion" id="menu-accordion-{{ $index }}">
                            <button type="button"
                            ...
                            <div id="menu-accordion-collapse-{{ $index }}"
                                ...
                                role="region" aria-labelledby="menu-accordion-{{ $index }}">
                                <ul class="hs-accordion-group pt-1 ps-7 space-y-1" data-hs-accordion-always-open>
                                    @foreach($menuItem['submenu'] ?? [] as $subIndex => $subItem)
                                    @if($subItem['type'] === 'link')
                                    <li>
                                        <a class="flex items-center gap-x-3.5 py-2 px-2.5 text-sm text-gray-800 rounded-lg hover:bg-gray-100 focus:outline-hidden focus:bg-gray-100 dark:bg-neutral-800 dark:hover:bg-neutral-700 dark:focus:bg-neutral-700 dark:text-neutral-200 {{ request()->is(ltrim($subItem['url'], '/')) ? 'bg-gray-100 dark:bg-neutral-700' : '' }}"
                                            href="{{ $subItem['url'] }}">
                                            {{ $subItem['text'] }}
                                        </a>
                                    </li>
                                    @elseif($subItem['type'] === 'accordion')
                                    {{-- Nested Accordion --}}
                                    <li class="hs-accordion" id="menu-accordion-{{ $index }}-{{ $subIndex }}">
                                        <button type="button"
                                            class="hs-accordion-toggle w-full text-start flex items-center gap-x-3.5 py-2 px-2.5 text-sm text-gray-800 rounded-lg hover:bg-gray-100 focus:outline-hidden focus:bg-gray-100 dark:bg-neutral-800 dark:hover:bg-neutral-700 dark:focus:bg-neutral-700 dark:text-neutral-200"
                                            aria-expanded="false"
                                        ...
                                        <div id="menu-accordion-collapse-{{ $index }}-{{ $subIndex }}"
                                            class="hs-accordion-content w-full overflow-hidden transition-[height] duration-300 hidden"
                                            role="region" aria-labelledby="menu-accordion-{{ $index }}-{{ $subIndex }}">
                                            <ul class="pt-1 ps-2 space-y-1">
                                                @foreach($subItem['submenu'] ?? [] as $subSubItem)
                                                @if($subSubItem['type'] === 'link')
                                                <li>
